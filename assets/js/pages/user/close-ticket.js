$("#table").DataTable({
    ...
    ajax: {
        type: "GET",
        url: "/user/ticket/closeticket/getdata"
    },
    columns: [
        { 
            data: "no_ticket",
        ...
        { 
            ...
            render: function(data, type, row){
                return `
                    <span class="badge bg-primary" onclick="closeTicket(${row.id})">CLOSE</span>
                `;
            }
        },
    ]
...
function viewTicket(ticketId) {
    ...
    $.ajax({
        url: `/user/ticket/closeticket/getById/${ticketId}`,
        type: 'GET',
        dataType: 'json', 
...
function closeTicket(ticketId) {
    Swal.fire({
        ...
        text: "Pastikan ticket sesuai dengan permintaan anda",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: `/user/ticket/closeticket/close/${ticketId}`, 
                type: 'POST', 
                dataType: 'json',
                success: function(response) {
                    Swal.close();
                    ...
                    if(response.status === 'success') {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: response.message || 'Tiket berhasil ditutup.',
                        }).then(() => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                            text: response.message || 'Terjadi kesalahan saat menutup tiket.',
                        });
                    }
                },
                error: function(xhr, status, error) {
                    ...
                    console.error('AJAX Error closing ticket:', error);
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal!',
